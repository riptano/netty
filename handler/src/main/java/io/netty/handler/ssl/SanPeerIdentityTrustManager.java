...
/**
 ...
 A decorator for {@link X509ExtendedTrustManager} that performs SAN-aware peer identity selection
 before delegating certificate verification to the underlying trust manager.

 delegate remains responsible for the actual certificate chain validation and endpoint
 verification, while this class adds the SAN-aware peer identity selection needed before that
 verification is performed.

 wrapper must remain usable anywhere an {@link X509ExtendedTrustManager} is expected by the JDK
 or OpenSSL SSL context setup paths. In particular, the extended trust manager API provides the
 {@code checkServerTrusted(..., SSLEngine)} callback, which gives this class access to the
 {@link SSLEngine} and its current peer host.

 callback inspects the peer certificate's SAN entries and determines the peer identity that
 should be used for endpoint verification. If the selected identity differs from the identity
 exposed by the original engine, the engine is wrapped so that {@code getPeerHost()} exposes
 the selected identity. Verification is then delegated to the underlying trust manager using
 that wrapped engine.

 {@code checkServerTrusted(..., SSLEngine)} callback required for this peer identity selection.
 Therefore, the class both wraps an {@link X509ExtendedTrustManager} for the actual verification
 and remains an {@link X509ExtendedTrustManager} so that the extended callback path is preserved.
 */
final class SanPeerIdentityTrustManager extends X509ExtendedTrustManager {
    ...
    static String resolvePeerIdentity(String peerHost, X509Certificate leafCertificate) throws CertificateException {
        ...
        if (sanTypes.hasDnsSans && isHostname(peerHost)) {
            logger.debug("Using hostname peer identity '{}' for DNS SAN verification", peerHost);
            return peerHost;
        }
        ...
        logger.debug("Reverse DNS lookup did not produce a hostname; using original peer identity '{}'", peerHost);
        return peerHost;
    }
